...
## 0. Упрощённый поток заявки артиста (используется текущим фронтом)

Фронтенд ходит на эти эндпоинты (роль пользователя остаётся `user` до одобрения модератором):

| Метод | Путь | Поведение |
|-------|------|------------|
| `GET` | `/api/users/me/artist-request` | Текущая заявка авторизованного пользователя. **404** или **204**, если заявки нет. Иначе JSON: минимум поля `id`, `status` (`pending` \| `approved` \| `rejected`), опционально `moderator_comment`, `artist_name`, `bio`. Допустимо оборачивать в `{ "request": { ... } }` — клиент нормализует. |
| `POST` | `/api/users/me/artist-request` | Тело: `{ "artist_name": "...", "bio": "..." }`. Создаёт заявку со статусом **`pending`** и **не** меняет `users.role` до модерации. Если у пользователя уже есть активная заявка в `pending` (или эквивалент), вернуть **409** и JSON `{ "message": "..." }` (например: «Заявка уже отправлена»). |

Модератор забирает очередь через уже описанный ниже `GET /api/moderation/artist-requests` и `POST /api/moderation/artist-requests/moderate`.

Статусы, которые фронт считает «на модерации» и блокирует повторную отправку: `pending`, `submitted`, `under_review`, `new`, `on_review`.
